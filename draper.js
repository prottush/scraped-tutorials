        ...
        if (type !== "rbpsd") {
            if (currTeam !== game.Team || i === data.length - 1) {
                teamChart.push({
                    team: currTeam,
                    gameE: game.Date,
                    gameS: teamChart.length === 0 ? data[1].Date : teamChart[teamChart.length - 1].gameE,
                });
                currTeam = game.Team;
            }
...
const scrapePBPTOT = async (fname, lname, hard = "soft") => {
    if (false) {
        ...
        try {
            const result = await doRequest(
                "https://api.pbpstats.com/get-game-logs/nba?Season=2021-22%2C2020-21%2C2019-20%2C2018-19%2C2017-18%2C2016-17%2C2015-16&SeasonType=Regular%20Season&EntityType=Player&EntityId=" + pID
            );
...
const scrapeTShot = async (team) => {
    try {
        ...
        await page.goto("https://www.pbpstats.com/game-logs/nba/team?Season=2021-22&SeasonType=Regular+Season&EntityId=" + tID + "&EntityType=Team&Table=Shooting&StatType=Totals");
        await page.waitForSelector(".line-numbers", { timeout: 20000 });
...
const scrapeTTo = async (team) => {
    try {
        ...
        const tID = teams[team + ""];
        await page.goto("https://www.pbpstats.com/game-logs/nba/team?Season=2021-22&SeasonType=Regular+Season&EntityId=" + tID + "&EntityType=Team&Table=Turnovers&StatType=Totals");
        await page.waitForSelector(".line-numbers", { timeout: 20000 });
